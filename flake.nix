{
  ...
  inputs = {
    ...
    # museld contains the ac3rf C++ library + Python bindings (player/ subdirectory).
    museld = {
      url = "github:staffanu/museld";
      flake = false;
    };
    # nanobind v2.4.0 — vendored so CMake FetchContent can run offline in the Nix sandbox.
    nanobind-src = {
  ...
  outputs = { self, nixpkgs, flake-utils, museld, nanobind-src }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        # Build the ac3rf Python extension (nanobind-based C++ module) from museld/player.
